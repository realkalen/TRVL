<?php
    ...
    function login(){
        ...
        // Stage 1: Preparation
        $query="SELECT * FROM trvl.passenger WHERE ID=? AND Pwd=?";
    ...
    function registerUser(){
        ...
        $genders=filterinput($conn,$_POST['Gender']);
        $type=filterinput($conn,$_POST['Type']);
        
        $bind=$stmt->bind_param("ississs",$userID,$password,$name,$batch,$dept,$genders,$type);
        if(!$bind){
            echo "Connection parameters failed: (" . $stmt->errno . ")" . $stmt->error;
        }
