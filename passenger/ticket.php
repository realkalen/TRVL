  ...
  <a href="index.php" id="profile">Your Profile<span class="glyphicon glyphicon-user"></span></a>
	...
	<div class="container">
		<div class="alert alert-success">
		  <strong>Yay! Your ticket is ready...</strong> It's time to TRVL :)
		</div>
	</div>
	<?php
	require 'db_init.php';
	...
	$bid = $_GET['bid'];
	$seat = $_GET['seat'];
	$sql_instance="SELECT * FROM trvl.seat_matrix JOIN trvl.routes ON trvl.seat_matrix.RID = trvl.routes.RID WHERE Passenger=".$userID." AND BID=".$bid." AND SeatNo=".$seat.";";
	$result = $conn->query($sql_instance);
	$row = $result->fetch_assoc();
	$qr_pass = '<<TRVL Digital Ticket>><Journey Date - '.$row['BusDate'].'><Route ID - '.$row['RID'].'><Seat Number - '.$row['SeatNo'].'><Passenger ID - '.$row['Passenger'].'><<TRVL!>>';
	echo '<center><div class="container-fluid">
					<div class="card bg-info text-white" style="width:30%">
					  ...
					  <div class="card-body">
					    <center><h3 class="card-title">TRVL Digital Ticket</h3>
