...
<body>
	...
	<?php
		$sql="SELECT Type FROM trvl.passenger WHERE ID='$userID'";
		$result = $conn->query($sql);
		$row = $result->fetch_assoc();
		$userType=$row['Type'];
		if($userType=='Student'){
			$sql_instance="SELECT * FROM trvl.student WHERE RollNo='$userID'";
			$result = $conn->query($sql_instance);
			$row = $result->fetch_assoc();
			echo '<center><div class="container-fluid">
							<div class="card bg-info pl-2" style="width:25%">
								<br><br>
								<div class="card-header"><h3>Your TRVL Profile</h3></div><br><br>
								<h4 class="card-body text-justify">&nbsp;&nbsp;&nbsp;&nbsp;Your Name&emsp;-&emsp;'.$row['SName'].'</h4>
									...
									<h4 class="card-body text-justify">&nbsp;&nbsp;&nbsp;&nbsp;Gender Group&emsp;-&emsp;'.$row['Gender'].'</h4>
									<br><br>
							</div>
							</div>
						</div><br><br>';
			...
			$sql_instance="SELECT * FROM trvl.faculty WHERE FID='$userID';";
			$result = $conn->query($sql_instance);
			$row = $result->fetch_assoc();
			echo '<center><div class="container-fluid">
							<div class="card bg-info pl-2" style="width:25%">
								...
								<h4 class="card-body text-justify">&nbsp;&nbsp;&nbsp;&nbsp;Your Name&emsp;-&emsp;'.$row['FName'].'</h4>
									...
									<h4 class="card-body text-justify">&nbsp;&nbsp;&nbsp;&nbsp;Gender&emsp;-&emsp;'.$row['Gender'].'</h4>
									<br><br>
							</div>
							</div>
						</div><br><br>';
		}else{
			$sql_instance="SELECT * FROM trvl.staff WHERE EID='$userID';";
			$result = $conn->query($sql_instance);
			$row = $result->fetch_assoc();
			echo '<center><div class="container-fluid">
							<div class="card bg-info pl-2" style="width:25%">
