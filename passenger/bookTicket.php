  ...
  <a href="profile.php" id="profile">Your Profile<span class="glyphicon glyphicon-user"></span></a>
	...
	<div class="container">
	    ...
	    <?php
			...
			$userType=$row['Type'];
			$sql_instance="SELECT * FROM trvl.bus_instances JOIN trvl.routes ON trvl.routes.RID=trvl.bus_instances.RID WHERE BusDate = CURDATE() ORDER BY DepTime ASC;";
		...
		<?php
			...
			$row=$result->fetch_assoc();
			$userType=$row['Type'];
			$sql_instance="SELECT * FROM trvl.bus_instances JOIN trvl.routes ON trvl.routes.RID=trvl.bus_instances.RID WHERE BusDate = CURDATE() + INTERVAL 1 DAY ORDER BY DepTime ASC;";
			$result = $conn->query($sql_instance);
			if ($result->num_rows > 0) {
				echo ' <table class="table table-hover"  style= "overflow-y:scroll; height:800px; display:block">
							<thead>
